...
static int __create_main_content(callui_view_mc_conf_h vd)
{
	...
	vd->manage_calls_ly = _callui_load_edj(vd->base_view.contents, EDJ_NAME, GRP_MANAGE_CALLS);
	CALLUI_RETURN_VALUE_IF_FAIL(vd->manage_calls_ly, CALLUI_RESULT_ALLOCATION_FAIL);
	elm_object_part_content_set(vd->caller_info, "manage_calls_icon_swallow", vd->manage_calls_ly);
	edje_object_signal_callback_add(_EDJ(vd->manage_calls_ly), "mouse,clicked,1", "btn", __manage_calls_btn_clicked_cb, vd);
...
static int __callui_view_multi_call_conf_ondestroy(call_view_data_base_t *view_data)
{
	...
	if (vd->manage_calls_ly) {
		elm_object_part_content_unset(vd->caller_info, "manage_calls_icon_swallow");
		edje_object_part_unswallow(_EDJ(vd->caller_info), vd->manage_calls_ly);
		evas_object_del(vd->manage_calls_ly);
		vd->manage_calls_ly = NULL;
	}
