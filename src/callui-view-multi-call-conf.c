...
static Eina_Bool __call_duration_timer_cb(void* data)
{
	...
	struct tm *new_tm = _callui_stp_get_call_duration(vd->base_view.ad->call_stp,
...
static callui_result_e __update_displayed_data(callui_view_mc_conf_h vd)
{
	...
	const callui_call_state_data_t *call_data = _callui_stp_get_call_data(ad->call_stp, CALLUI_CALL_DATA_TYPE_ACTIVE);
	if (!call_data) {
		call_data = _callui_stp_get_call_data(ad->call_stp,
	...
	if (is_held) {
		...
		vd->base_view.call_duration_tm = _callui_stp_get_call_duration(ad->call_stp, CALLUI_CALL_DATA_TYPE_ACTIVE);
