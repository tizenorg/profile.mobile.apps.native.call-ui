...
//
//	if (active && !active->is_dialing && !incom) {
//		if (held) {
//			_callui_common_update_call_duration(held->start_time);
//		} else {
//			_callui_common_update_call_duration(active->start_time);
//		}
//	}
...
bool _callui_common_is_extra_volume_available(void *data)
{
	...
	callui_audio_state_type_e audio_state =
			_callui_sdm_get_audio_state(ad->call_sdm);
...
bool _callui_is_on_handsfree_mode()
{
	...
	callui_audio_state_type_e type = _callui_sdm_get_audio_state(ad->call_sdm);
