...
static int __callui_view_incoming_call_oncreate(call_view_data_base_t *view_data, void *appdata)
{
	...
	res = elm_win_keygrab_set(ad->win, CALLUI_KEY_SELECT, 0, 0, 0, ELM_WIN_KEYGRAB_TOPMOST);
	if (res) {
		dbg("KEY_SELECT key grab failed [%d]", res);
...
static int __callui_view_incoming_call_ondestroy(call_view_data_base_t *view_data)
{
	...
	if (vd->lock_accept) {
		evas_object_del(vd->lock_accept);
		vd->lock_accept = NULL;
	}
	if (vd->lock_reject) {
		evas_object_del(vd->lock_reject);
		vd->lock_reject = NULL;
	}
	...
	if (vd->reject_msg_layout) {
		evas_object_del(vd->reject_msg_layout);
		vd->reject_msg_layout = NULL;
	}

	elm_object_signal_emit(vd->base_view.contents, "mt_circle_bg_hide", "mt_view");
	_callui_common_reset_main_ly_text_fields(vd->base_view.contents);

	vd->base_view.contents = elm_object_part_content_get(ad->main_ly, "elm.swallow.content");
...
static int __create_main_content(callui_view_incoming_call_h vd)
{
	...
	elm_object_signal_emit(vd->base_view.contents, "mt_circle_bg_show", "mt_view");

	_callui_show_caller_info_status(ad, _("IDS_CALL_BODY_INCOMING_CALL"));

	int res = _callui_view_circle_create_accept_layout(ad, vd);
	CALLUI_RETURN_VALUE_IF_FAIL(res == CALLUI_RESULT_OK, res);
	res = _callui_view_circle_create_reject_layout(ad, vd);
	CALLUI_RETURN_VALUE_IF_FAIL(res == CALLUI_RESULT_OK, res);
