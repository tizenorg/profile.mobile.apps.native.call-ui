...
static callui_result_e __callui_manager_init(callui_manager_h cm_h);
static void __callui_manager_deinit(callui_manager_h cm_h);
...
static void __end_call_called_handle_func(_callui_listener_t *listener, va_list args);

static callui_dial_status_e __convert_cm_dial_status(cm_dial_status_e type);
static void __dial_status_handle_func(_callui_listener_t *listener, va_list args);
...
static void __dial_status_cb(cm_dial_status_e dial_status, void *user_data)
{
	...
	callui_manager_h cm_h = user_data;
	callui_dial_status_e status = __convert_cm_dial_status(dial_status);

	dbg("Dialing status [%d]", dial_status);

	_callui_listeners_coll_call_listeners(&cm_h->dial_status_lc, status);
}

static callui_result_e __callui_manager_init(callui_manager_h cm_h)
{
	callui_result_e res = _callui_utils_convert_cm_res(cm_init(&cm_h->cm_client));
	CALLUI_RETURN_VALUE_IF_FAIL(res == CALLUI_RESULT_OK, res);

	_callui_listeners_coll_init(&cm_h->end_call_lc);
	_callui_listeners_coll_init(&cm_h->dial_status_lc);

	cm_h->sound_manager = _callui_sdm_create(cm_h->cm_client);
	CALLUI_RETURN_VALUE_IF_FAIL(cm_h->sound_manager, CALLUI_RESULT_FAIL);

	cm_h->state_provider = _callui_stp_create(cm_h->cm_client);
	CALLUI_RETURN_VALUE_IF_FAIL(cm_h->state_provider, CALLUI_RESULT_FAIL);

	res = _callui_utils_convert_cm_res(cm_set_dial_status_cb(cm_h->cm_client, __dial_status_cb, cm_h));
	CALLUI_RETURN_VALUE_IF_FAIL(res == CALLUI_RESULT_OK, res);
...
static void __callui_manager_deinit(callui_manager_h cm_h)
{
	_callui_listeners_coll_deinit(&cm_h->end_call_lc);
	_callui_listeners_coll_deinit(&cm_h->dial_status_lc);

	cm_unset_dial_status_cb(cm_h->cm_client);

	if (cm_h->state_provider) {
		_callui_stp_destroy(cm_h->state_provider);
		cm_h->state_provider = NULL;
	}

	if (cm_h->sound_manager) {
		_callui_sdm_destroy(cm_h->sound_manager);
		cm_h->sound_manager = NULL;
	}

	if (cm_h->cm_client) {
		cm_deinit(cm_h->cm_client);
		cm_h->cm_client = NULL;
	}
}
...
{
	callui_manager_h cm_h = calloc(1, sizeof(_callui_manager_t));
	CALLUI_RETURN_NULL_IF_FAIL(cm_h);

	callui_result_e res = __callui_manager_init(cm_h);
	if (res != CALLUI_RESULT_OK) {
		__callui_manager_deinit(cm_h);
		FREE(cm_h);
	}
	return cm_h;
}
...
callui_result_e _callui_manager_dial_voice_call(callui_manager_h cm_h, const char *number, callui_sim_slot_type_e sim_slot)
{
	CALLUI_RETURN_VALUE_IF_FAIL(cm_h, CALLUI_RESULT_INVALID_PARAM);
	...
	return _callui_utils_convert_cm_res(
			cm_dial_call(cm_h->cm_client, temp_number, CM_CALL_TYPE_VOICE, __convert_callui_sim_type(sim_slot)));
}

callui_result_e _callui_manager_end_call(callui_manager_h cm_h, unsigned int call_id, callui_call_release_type_e release_type)
{
	CALLUI_RETURN_VALUE_IF_FAIL(cm_h, CALLUI_RESULT_INVALID_PARAM);

	callui_result_e res = _callui_utils_convert_cm_res(
			cm_end_call(cm_h->cm_client, call_id, __convert_app_release_type(release_type)));

	if (res == CALLUI_RESULT_OK) {
		_callui_listeners_coll_call_listeners(&cm_h->end_call_lc, call_id, release_type);
	}
...
callui_result_e _callui_manager_split_call(callui_manager_h cm_h, unsigned int call_id)
{
	...
	return _callui_utils_convert_cm_res(
			cm_split_call(cm_h->cm_client, call_id));
}
...
callui_result_e _callui_manager_answer_call(callui_manager_h cm_h, callui_call_answer_type_e ans_type)
{
	...
	return _callui_utils_convert_cm_res(
			cm_answer_call(cm_h->cm_client, __convert_app_answer_type(ans_type)));
		...
		callui_end_call_called_cb cb_func, void *cb_data)
{
	...
	return _callui_listeners_coll_add_listener(&cm_h->end_call_lc,
			__end_call_called_handle_func, cb_func, cb_data);
}
		...
		callui_end_call_called_cb cb_func, void *cb_data)
{
	...
	return _callui_listeners_coll_remove_listener(&cm_h->end_call_lc, cb_func, cb_data);
}
		...
		callui_dial_status_cb cb_func, void *cb_data)
{
	...
	return _callui_listeners_coll_add_listener(&cm_h->dial_status_lc,
			__dial_status_handle_func, cb_func, cb_data);
}
		...
		callui_dial_status_cb cb_func, void *cb_data)
{
	...
	return _callui_listeners_coll_remove_listener(&cm_h->dial_status_lc, cb_func, cb_data);
}
