...
struct _callui_qp_mc {
	...
	bool is_disabled[QP_MC_BTN_TYPE_MAX];
...
typedef void (*btn_update_func)(callui_qp_mc_h qp, Eina_Bool is_disable);
...
static void __update_call_btn(callui_qp_mc_h qp, Eina_Bool is_disable);
static void __update_resume_btn(callui_qp_mc_h qp, Eina_Bool is_disable);
static void __update_mute_btn(callui_qp_mc_h qp, Eina_Bool is_disable);
static void __update_speaker_btn(callui_qp_mc_h qp, Eina_Bool is_disable);
...
void __audio_state_changed_cb(void *user_data, callui_audio_state_type_e state)
{
	...
	if (qp->is_activated) {
		__update_speaker_btn(qp, qp->is_disabled[QP_MC_BTN_TYPE_SPEAKER]);
	}
}
...
void __mute_state_changed_cb(void *user_data, bool is_enable)
{
	...
	if (qp->is_activated) {
		__update_mute_btn(qp, qp->is_disabled[QP_MC_BTN_TYPE_MUTE]);
	}
}
...
static void __update_layout_components(callui_qp_mc_h qp)
{
	...
	__update_call_btn(qp, EINA_TRUE);
	__update_speaker_btn(qp, EINA_FALSE);

	if (incom) {
		/* Incoming call */
		__update_comp_status(qp, eo, "incoming_call", incom);
		__update_mute_btn(qp, EINA_TRUE);
		__update_speaker_btn(qp, EINA_TRUE);
		__update_call_btn(qp, EINA_FALSE);
		...
		__update_mute_btn(qp, EINA_FALSE);
		...
		__update_mute_btn(qp, EINA_TRUE);
		__update_resume_btn(qp, EINA_TRUE);
...
static void __update_speaker_btn(callui_qp_mc_h qp, Eina_Bool is_disable)
{
	...
	qp->is_disabled[QP_MC_BTN_TYPE_SPEAKER] = is_disable;
}
...
static void __update_mute_btn(callui_qp_mc_h qp, Eina_Bool is_disable)
{
	...
	qp->is_disabled[QP_MC_BTN_TYPE_MUTE] = is_disable;
}
