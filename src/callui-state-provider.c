...
static void __call_event_cb(cm_call_event_e call_event, cm_call_event_data_t *call_state_data, void *user_data)
{
	...
	bool is_need_call_event_listeners = true;
	...
	{
		callui_call_state_data_t *temp = stp->st_data_list[CALLUI_CALL_DATA_TYPE_INCOMING];

		if (temp && call_id == temp->call_id) {
			__update_stp_all_call_data(stp, cm_incom, cm_active, cm_held);
		} else if (cm_incom == NULL && cm_active == NULL && cm_held == NULL) {
			...
			is_need_call_event_listeners = false;
	...
	if (is_need_call_event_listeners) {
...
callui_result_e _callui_stp_get_call_data(callui_state_provider_h stp,
		...
		const callui_call_state_data_t **call_data)
{
	...
	CALLUI_RETURN_VALUE_IF_FAIL((call_data_type >= CALLUI_CALL_DATA_TYPE_ACTIVE &&
			call_data_type < CALLUI_CALL_DATA_TYPE_MAX), CALLUI_RESULT_INVALID_PARAM);

	*call_data = stp->st_data_list[call_data_type];
...
callui_result_e _callui_stp_get_conference_call_list(callui_state_provider_h stp, Eina_List **conf_call_list)
{
	CALLUI_RETURN_VALUE_IF_FAIL(stp, CALLUI_RESULT_INVALID_PARAM);
	CALLUI_RETURN_VALUE_IF_FAIL(conf_call_list, CALLUI_RESULT_INVALID_PARAM);
	...
	if (call_list) {
		...
		if (res != CALLUI_RESULT_OK) {
			...
			eina_list_free(res_list);
		} else {
			*conf_call_list = res_list;
		}
...
callui_result_e _callui_stp_get_call_duration(callui_state_provider_h stp,
		callui_call_data_type_e call_data_type,
		struct tm *time)
{
	CALLUI_RETURN_VALUE_IF_FAIL(stp, CALLUI_RESULT_INVALID_PARAM);
	CALLUI_RETURN_VALUE_IF_FAIL((call_data_type >= CALLUI_CALL_DATA_TYPE_ACTIVE &&
			call_data_type < CALLUI_CALL_DATA_TYPE_MAX), CALLUI_RESULT_INVALID_PARAM);
	CALLUI_RETURN_VALUE_IF_FAIL(stp->st_data_list[call_data_type], CALLUI_RESULT_FAIL);
